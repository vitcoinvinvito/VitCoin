...
// Copyright (c) 2013 The VitCoin developers
...
uint256 hashGenesisBlock("0x00000b51014660813a182cc6a4b1d502470b203b0182bb00766886c0de1e32f3");
static const unsigned int timeGenesisBlock = 1477809000;
...
const string strMessageMagic = "VitCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    ...
    int64 nSubsidy = 58 * COIN;
	if (nHeight == 1 ) 
    {
        nSubsidy = 153980000 * COIN;
    }
    else if (nHeight > 1 && nHeight < 4204801)
    {
        nSubsidy >>= nHeight / ( 1440 * 365 * 2 );
    }
    else if (nHeight > 4204800 && nHeight < 4730401)
    {
        nSubsidy = 1702000 * COIN / ( 1440 * 365 );
    }
    else
        nSubsidy = 0 * COIN;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Oct 30, 2016. The VitCoin was released, and hoping it is a great coin.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0xf2a4a4aa9c48cb6db936b4a269c0bac62a12e0c203d427937511e1acdc523753"));
...
unsigned char pchMessageStart[4] = { 0x14, 0x10, 0x0e, 0x1e };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VitCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VitCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("VitCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("VitCoin-miner");
    ...
    try { loop {
        ...
        printf("Running VitCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("VitCoinMiner terminated\n");
        throw;
    }
}
